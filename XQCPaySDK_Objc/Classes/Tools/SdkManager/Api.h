...
+ (void)payRequsetAmount:(CGFloat)amount payType:(NSString *)type bizCode:(NSString *)bizCode Body:(NSString *)body orderId:(NSString *)orderId iousCode:(NSString *)iousCode FeeType:(feeType)myFeeType viewController:(UIViewController *)vc error:(void(^)(NSString *error))errorMsg;
+ (void)checkPayPwd:(NSString *)password reuslt:(void(^)(PasswordModel *model))result;
+ (void)queryOrder:(NSString *)orderId PayType:(NSString *)paytype reuslt:(void(^)(ResponseModel *model))result;
@end
